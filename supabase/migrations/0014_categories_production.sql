-- 0014: Categories become self-contained scrape definitions (business type +
-- location, geocoded in-browser). Manual "Run" = a one-off scraped schedule.
-- Removes the 5 demo consumer categories. Adds leads.country. Rewires
-- claim_and_enqueue to read geo/search from the CATEGORY; upsert_lead writes country.

-- ---- categories = scrape definitions ----
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS search_terms TEXT;             -- e.g. "dentist"
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS osm_filter TEXT;               -- optional raw Overpass tag, overrides keywords
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS geo TEXT;                       -- "Denver, CO"
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS country TEXT;                   -- "United States"
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS lat NUMERIC;
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS lng NUMERIC;
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS source_key TEXT DEFAULT 'osm_overpass';
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS radius_m INT DEFAULT 6000;
ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS lead_count INT DEFAULT 12;

-- Remove the demo consumer categories (fully-custom going forward).
DELETE FROM public.categories WHERE key IN ('real_estate_buyer','real_estate_seller','mortgage','insurance','b2b');

-- ---- leads: country ----
ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS country TEXT;

-- ---- schedules: one-off "Run now" ----
ALTER TABLE public.schedules ADD COLUMN IF NOT EXISTS one_off BOOLEAN NOT NULL DEFAULT false;

-- ---- claim_and_enqueue: build work-items from the CATEGORY; disable one-offs ----
...
BEGIN
  ...
  LOOP
    ...
    IF c.key IS NULL OR c.lat IS NULL OR c.lng IS NULL THEN CONTINUE; END IF;  -- need a geocoded location
    IF NOT EXISTS (
    ...
    items := items || jsonb_build_object(
      ...
      'country', c.country, 'radius_m', COALESCE(c.radius_m,6000), 'count', COALESCE(r.lead_count, c.lead_count, 12));
  END LOOP;
...
-- ---- upsert_lead: add country (keeps 0012 socials merge) ----
...
BEGIN
  INSERT INTO public.leads (
    category, name, email, phone, linkedin_url, socials, website, company, location, country,
    ...
    p->>'website', p->>'company', p->>'location', p->>'country',
